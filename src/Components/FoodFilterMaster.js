...
class FoodFilterMaster extends Component {
    ...
    componentDidMount () {
        Notiflix.Loading.Init({
          ...
          GetApiCall.getRequest("GetFoodFilterList").then(resultdes =>
    ...
    onPost = () =>{
        ...
        },"AddFoodFilterMaster").then((resultFoodF) =>
        resultFoodF.json().then(objfoodC => {
            if(resultFoodF.status == 200 || resultFoodF.status == 201){
                this.props.setclearfoodfilter()
                Notiflix.Loading.Remove();
                Notiflix.Notify.Success('Food filter successfully added.')
                window.location.reload()
            }else
            {
                Notiflix.Loading.Remove();
                Notiflix.Notify.Failure('Food filter already present.')
            }
         ...
         this.state = {
          ...
          savefood(){
              if(this.props.foodcredentails.FoodFilter!=''){
                ...
                Notiflix.Notify.Failure('Please enter food filter name.')
             }
          }
          ...
          updatefood(){
            if(this.props.foodcredentails.FoodFilter!=''){
                ...
                var details=JSON.parse(login)
                PostApiCall.postRequest ({
                    foodfilterid : this.state.FoodId,
                    filter : this.props.foodcredentails.FoodFilter,
                    status : this.state.Status,
                    updatedby : details[0].fld_staffid,
                    updatedon : moment().format('lll')
                },"UpdateFoodFilterMaster").then((resultFoodF) =>
                resultFoodF.json().then(objfoodC => {
                    if(resultFoodF.status == 200 || resultFoodF.status == 201){
                        this.props.setclearfoodfilter()
                        Notiflix.Loading.Remove();
                        Notiflix.Notify.Success('Food filter successfully updated.')
                        window.location.reload()
                    }else
                    {
                        Notiflix.Loading.Remove();
    ...
    <div class="modal-content modelcontent2">
      <div class="modal-header">
        <h4 class="modal-title">Add New Food Filter</h4>
      </div>
    ...
    <div class="modal-content modelcontent2">
      <div class="modal-header">
        <h4 class="modal-title">Update Food Filter</h4>
      </div>
      <div class="modal-body">
            <div class="col-md-12">
            <div class="form-group mb-3">
                        ...
                        <div class="col-md-12">
                            <nav aria-label="breadcrumb" class="float-right mt-1">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">Food Filter Master</li>
                                </ol>
                            </nav>
                            <h4 class="mb-1 mt-0">Food Filter Master</h4>
                        </div>
                    </div> 
                        ...
                        <div class="col-12">
                            <div class="card">
                                ...
                                {this.state.FoodFilterData.length == 0 ? 
                                 <tr><td colSpan={4} style={{textAlign:'center'}}>No Food Filter Available</td></tr> : 
                                 ''}
                                       ...
                                       <script src="assets/libs/datatables/dataTables.bootstrap4.min.js"></script>
                                           ...
                                           onClick={()=>{
                                            confirmAlert({
                                              ...
                                              message: 'Are you sure you want to delete food filter.',
                                              buttons: [
                                                {
                                                  label: 'Yes',
                                                  onClick: () => {
                                          ...
                                          PostApiCall.postRequest({
                                            ...
                                            filterid : data.fld_id,
                                              ...
                                              if(results.status == 200 || results.status==201){
                                                  ...
                                                  Notiflix.Loading.Remove()
                                                  Notiflix.Notify.Success('Food filter successfully deleted.')
                                                  window.location.reload()
                                              }else
                                              {
                                                  Notiflix.Loading.Remove()
