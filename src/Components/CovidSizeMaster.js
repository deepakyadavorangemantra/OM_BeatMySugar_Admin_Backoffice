...
class CovidSizeMaster extends Component {
    ...
    componentDidMount(){
        Notiflix.Loading.Init({
          ...
          GetApiCall.getRequest("GetCovidSizeList").then(resultdes =>
    ...
    onPost = () =>{
      ...
      var details=JSON.parse(login)
        ...
        },"AddCovidSizeMaster").then((resultSocksS) =>
        resultSocksS.json().then(objSocks => {
            if(resultSocksS.status == 200 || resultSocksS.status == 201){
                this.props.setclearsockssize()
                Notiflix.Loading.Remove();
                Notiflix.Notify.Success('Size successfully added.')
                window.location.reload()
            }else
            {
              Notiflix.Loading.Remove();
              Notiflix.Notify.Failure('Size already present.')
            }
        })
        )
    }
            ...
            if(this.props.sockssizecredentails.SocksSize!=''){
              ...
              var details=JSON.parse(login)
                PostApiCall.postRequest ({
                    ...
                    updatedby : details[0].fld_staffid,
                    updatedon : moment().format('lll')
                },"UpdateCovidSizeMaster").then((resultSocksS) =>
                resultSocksS.json().then(objSocks => {
                    if(resultSocksS.status == 200 || resultSocksS.status == 201){
                        this.props.setclearsockssize()
                        Notiflix.Loading.Remove();
                        Notiflix.Notify.Success('Size successfully updated.')
                        window.location.reload()
                    }else
                    {
                      Notiflix.Loading.Remove();
                        ...
                        <div class="col-md-12">
                            <nav aria-label="breadcrumb" class="float-right mt-1">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">Covid Size Master</li>
                                </ol>
                            </nav>
                            <h4 class="mb-1 mt-0">Covid Size Master</h4>
                        </div>
                    </div> 
                        ...
                        <div class="col-12">
                            <div class="card">
                                ...
                                {this.state.SocksSData.length == 0 ? 
                                 <tr><td colSpan={4} style={{textAlign:'center'}}>No Covid Size Available</td></tr> : 
                                 ''} 
                                       ...
                                       <script src="assets/libs/datatables/dataTables.bootstrap4.min.js"></script>
                                           ...
                                           onClick={()=>{
                                            confirmAlert({
                                              ...
                                              message: 'Are you sure you want to delete covid size.',
                                              buttons: [
                                                {
                                                  label: 'Yes',
                                                  onClick: () => {
                                          ...
                                          PostApiCall.postRequest({
                                            ...
                                            covidsizeid : data.fld_id,
                                           
                                        
                                            },"DeleteCovidSizeMaster").then((results) => 
                                              ...
                                              if(results.status == 200 || results.status==201){
                                                  ...
                                                  Notiflix.Loading.Remove()
                                                  Notiflix.Notify.Success('Size successfully deleted.')
                                                  window.location.reload()
                                              }else
                                              {
                                                  Notiflix.Loading.Remove()
