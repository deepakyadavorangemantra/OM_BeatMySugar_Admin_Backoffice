...
export default function Products() {
    ...
    const {statusValue,categoryValue}=state

   let newFoodArray=[]
       let newFootArray=[]
       let newSocksArray=[]
       let sumOfItemSales=0;
       let sumOfOrderCount=0;
       let sumOfItemSalesFoot=0
      let sumOfOrderCountFoot=0
      let sumOfItemSalesSocks=0
      let sumOfOrderCountSocks=0
      let refFoodArray=[]

     
      let group=[];
      if(state.startDate!==null&&state.endDate!==null){
         group=FoodData.filter(product=>{
        ...
        if(moment(moment(product.fld_updatedon).format('ll')).isBetween(moment(state.startDate).format('ll'),moment(state.endDate).format('ll'))){
            ...
            r[a.fld_foodid] = [...r[a.fld_foodid] || [], a];
            return r;
           }, []);
       ...
       group.map(data=>{
         ...
         sumOfItemSales= data.reduce((sum, currentValue) => {
              let current =currentValue.fld_itemsales===null?0:currentValue.fld_itemsales
            return sum +current ;
             }, 0);
         sumOfOrderCount= data.reduce((sum, currentValue) => {
              let current =currentValue.fld_itemordercount===null?0:currentValue.fld_itemordercount
            return sum +current ;
             }, 0);
           newFoodArray.push({...data[0],
              refFoodArray:refFoodArray,
                fld_itemordercount:sumOfOrderCount,
                fld_itemsales:sumOfItemSales,
               
            })

        //    console.log(sumOfItemSales,"SaleDome ")
        //    console.log(data)
        //    data[0].map(dat=>{
        //        console.log("group",dat)
        //    })
       })
...
//   console.log(newFoodArray,"newA")
    ...
    let   groupFoot =[];
       ...
       if(state.startDate!==null&&state.endDate!==null){
        groupFoot=FootsData.filter(product=>{
        // console.log(moment(moment(product.fld_updatedon).format('ll')).isBetween(moment(state.startDate).format('ll'),moment(state.endDate).format('ll')));
       if(moment(moment(product.fld_updatedon).format('ll')).isBetween(moment(state.startDate).format('ll'),moment(state.endDate).format('ll'))){
        ...
        r[a.fld_footid] = [...r[a.fld_footid] || [], a];
        return r;
       }, []);
        ...
        groupFoot = FootsData.reduce((r, a) => {
...
// console.log(groupFoot)

       groupFoot.map(data=>{
    //    console.log(data)
        sumOfItemSalesFoot= data.reduce((sum, currentValue) => {
             let current =currentValue.fld_itemsales===null?0:currentValue.fld_itemsales
           return sum +current ;
            }, 0);
        sumOfOrderCountFoot= data.reduce((sum, currentValue) => {
             let current =currentValue.fld_itemordercount===null?0:currentValue.fld_itemordercount
           return sum +current ;
            }, 0);
          newFootArray.push({...data[0],
            
               fld_itemordercount:sumOfOrderCountFoot,
               fld_itemsales:sumOfItemSalesFoot,
    ...
    if(state.startDate!==null&&state.endDate!==null){
        groupSocks=SocksData.filter(product=>{
        // console.log(moment(moment(product.fld_updatedon).format('ll')).isBetween(moment(state.startDate).format('ll'),moment(state.endDate).format('ll')));
       if(moment(moment(product.fld_updatedon).format('ll')).isBetween(moment(state.startDate).format('ll'),moment(state.endDate).format('ll'))){
        ...
        r[a.fld_socksid] = [...r[a.fld_socksid] || [], a];
        return r;
       }, []);
        ...
        groupSocks = SocksData.reduce((r, a) => {
       ...
       groupSocks.map(data=>{
           if(data.length>0){
               sumOfItemSalesSocks= data.reduce((sum, currentValue) => {
             let current =currentValue.fld_itemsales===null?0:currentValue.fld_itemsales
           return sum +current ;
            }, 0); 
          
           sumOfOrderCountSocks= data.reduce((sum, currentValue) => {
             let current =currentValue.fld_itemordercount===null?0:currentValue.fld_itemordercount
           return sum +current ;
            }, 0);
          ...
          newSocksArray.push({...data[0],
            
               fld_itemordercount:sumOfOrderCountSocks,
               fld_itemsales:sumOfItemSalesSocks,
              
           })

        //   console.log(sumOfItemSalesSocks,"SaleDome ")
       //    console.log(data)
       //    data[0].map(dat=>{
       //        console.log("group",dat)
       //    })
       ...
       useEffect(() => {
          setFoodData(newFoodArray)
          setFootsData(newFootArray)
          setSockssData(newSocksArray)
       }, [newFoodArray.length,newFootArray.length,newSocksArray.length,statusValue,categoryValue])

       
    //    console.log(newFoodArray,"New ARRAY")
const  Dropdown=() =>{
        ...
        <div class="row">
            ...
            <tbody> 
                {ProductData.map(product=>{
                   return <ProductsExtraGrids itemSku={product.fld_sku} itemName={product.fld_mastername}   NoofItemsOrdered={product.fld_itemordercount} 
