...
class FoodCategory extends Component {
    ...
    componentDidMount () {
        ...
        Notiflix.Loading.Init({
          ...
          GetApiCall.getRequest("Get_AccessoriesCategoryList").then(resultdes =>
            resultdes.json().then(obj => {
              console.log(obj.data)
                this.setState({
                AccessorieslistData : obj.data
              })
    ...
    onPost = () =>{
      ...
      var details=JSON.parse(login)
        ...
        resultFoodC.json().then(objfoodC => {
            ...
            Notiflix.Notify.Success('Accessories category successfully added.')
            window.location.reload()
            }else{
              Notiflix.Loading.Remove();
              Notiflix.Notify.Failure('Accessories category already present.')
            }
        })
        )
    }
     ...
     constructor(props){
        ...
        Savefoodcategory(){
          if(this.state.CategoryName!=''){
            ...
            Notiflix.Notify.Failure('Please enter accessories category abbrevation.')
         }
          }
          else{
            Notiflix.Notify.Failure('Please enter accessories category name.')
         }
      }
      ...
      Updatefoodcategory(){
        if(this.state.CategoryName!=''){
          ...
          var login=localStorage.getItem('LoginDetail');
          var details=JSON.parse(login)
        // console.log(details[0].fld_staffid)
          PostApiCall.postRequest ({

              accessoriesid : this.state.AccessoriesId,
              category : this.state.CategoryName,
              abv : this.state.Abv,
              status : this.state.Status,
              updatedon : moment().format('lll'),
          ...
          resultFoodC.json().then(objfoodC => {
              ...
              Notiflix.Loading.Remove();
              Notiflix.Notify.Success('Accessories category successfully updated.')
              window.location.reload()
              }else{
                Notiflix.Loading.Remove();
                Notiflix.Notify.Failure('Accessories category already present.')
    ...
    <div class="modal-content modelcontent3">
      <div class="modal-header">
        <h4 class="modal-title">Add New Accessories Category</h4>
      </div>
                        ...
                        <div class="col-md-12">
                            <nav aria-label="breadcrumb" class="float-right mt-1">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">Accessories Category Master</li>
                                </ol>
                            </nav>
                            <h4 class="mb-1 mt-0">Accessories Category Master</h4>
                        </div>
                    </div> 
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                ...
                                {this.state.AccessorieslistData.length == 0 ? 
                                 <tr><td colSpan={5} style={{textAlign:'center'}}>No Accessories Category Available</td></tr> : 
                                 ''}

                                    {this.state.AccessorieslistData.map((data,index)=>(
                                       ...
                                       <script src="assets/libs/datatables/dataTables.bootstrap4.min.js"></script>
                                           ...
                                           onClick={()=>{
                                            confirmAlert({
                                              ...
                                              message: 'Are you sure you want to delete accessories category.',
                                              buttons: [
                                                {
                                                  label: 'Yes',
                                                  onClick: () => {
                                          ...
                                          PostApiCall.postRequest({
                                            ...
                                            accessoriesid : data.fld_id,
                                            ...
                                            },"DeleteAccessoriesCategoryMaster").then((results) => 
                                              ...
                                              if(results.status == 200 || results.status==201){
                                                  ...
                                                  Notiflix.Loading.Remove()
                                                  Notiflix.Notify.Success('Accessories category successfully deleted.')
                                                  window.location.reload()
                                              }else
                                              {
                                                  Notiflix.Loading.Remove()
