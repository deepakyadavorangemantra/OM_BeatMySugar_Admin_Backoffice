...
componentDidMount(){
    ...
    var offerDt =    JSON.parse(localStorage.getItem('FestiveOffersDetails'))

    //  console.log(offerDt)

     this.setState({...this.state,
        Title:offerDt.fld_title,
        showOnWebsite : offerDt.fld_showonwebsite,
        offerid : offerDt.fld_id
    })

    PostApiCall.postRequest({
        offerid:offerDt.fld_id,
    },'GetFestiveOfferProductMapping').then((results) => 
    ...
    GetApiCall.getRequest("GetFoodItemData").then(resultdes =>
        resultdes.json().then(objfilter =>{
            ...
            for(var i = 0 ;i<Object.keys(obj.data).length;i++){
                for(var j = 0 ;j<Object.keys(objfilter.data).length;j++){

                    if(obj.data[i].fld_productid == objfilter.data[j].value && obj.data[i].fld_category == 'Food'){

                        
                        fdfilter.push(objfilter.data[j])
            ...
            GetApiCall.getRequest("GetFootwearItemData").then(resultdes =>
                resultdes.json().then(objfilterft =>{
                
                    this.setState({...this.state,
                        FOotwareFilterData:objfilterft.data,
                    })
                    ...
                    for(var i = 0 ;i<Object.keys(obj.data).length;i++){
                        for(var j = 0 ;j<Object.keys(objfilterft.data).length;j++){
        
                            if(obj.data[i].fld_productid == objfilterft.data[j].value && obj.data[i].fld_category == 'Footwear'){
        
                                
                                ftfilter.push(objfilterft.data[j])
                    ...
                    this.setState({
                        FOotwareFilter : ftfilter
                    })
        
                }))

                GetApiCall.getRequest("GetSocksItemData").then(resultdes =>
                    resultdes.json().then(objfiltersk =>{
                    
                        this.setState({...this.state,
                            SocksFilterData:objfiltersk.data,
                        })
                        ...
                        for(var i = 0 ;i<Object.keys(obj.data).length;i++){
                            for(var j = 0 ;j<Object.keys(objfiltersk.data).length;j++){
            
                                if(obj.data[i].fld_productid == objfiltersk.data[j].value && obj.data[i].fld_category == 'Socks'){
            
                                    
                                    skfilter.push(objfiltersk.data[j])
                    ...
                    title:this.state.Title,
                     ...
                     updatedby: details[0].fld_staffid,
                      updatedon:moment(new Date()).format('LLL')
                          },"UpdateFestiveOfferMaster").then((results) => 
           ...
           offerid:this.state.offerid,
                ...
                if(results1.status == 200 || results1.status==201){
                    ...
                    if(fcount == this.state.SOcksFilter.length){
                       ...
                       Notiflix.Loading.Remove()
                       Notiflix.Notify.Success('Offer updated successfully')
                        ...
                        <div className="row page-title">
                            <div className="col-md-12">
                              ...
                              <li class="breadcrumb-item active" aria-current="page">View Offer</li>
                                ...
                                <h4 class="mb-1 mt-0">View New Festive Offer</h4>
                            </div>
                        </div>
                                                                ...
                                                                <div className='row'>
                                                                    ...
                                                                    <div className="col-md-12">
                                                                        <div className="btn-toolbar p-1  sw-toolbar sw-toolbar-top justify-content-right" style={{ float: 'right' }}>
                                                                            ...
                                                                            //     })
                                                                            //   }}
                                                                              onClick={this.submitHandler.bind(this)}>Update Offer</button>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
