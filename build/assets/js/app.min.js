...
function (n) {
	...
	t.prototype._resetSidebarScroll = function () {
		...
		n(".button-menu-mobile").on("click", function (t) {
			t.preventDefault(), "topnav" === e.$body.data("layout") ? (n(this).toggleClass("open"), n("#topnav-menu-content").slideToggle(400)) : (e.$body.toggleClass("sidebar-enable"), 768 <= e.$window.width() ? e.$body.toggleClass("left-side-menu-condensed") : e.$body.removeClass("left-side-menu-condensed"), e._resetSidebarScroll())
		});
		var t = e.$body.data("layout");
		if (n("#menu-bar").length)
			if ("topnav" !== t) new MetisMenu("#menu-bar"), e._resetSidebarScroll(), n("#menu-bar a").each(function () {
		...
		768 <= this.$window.width() && this.$window.width() <= 1024 ? this.$body.addClass("left-side-menu-condensed") : 1 != this.$body.data("left-keep-condensed") && this.$body.removeClass("left-side-menu-condensed"), this.$body.hasClass("scrollable-layout") && n("#sidebar-menu").removeClass("slimscroll-menu")
	}, t.prototype.init = function () {
		var e = this;
